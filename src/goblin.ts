...
export default class Goblin implements Pedestrian
{
    ...
    constructor (phaser : NorthPoleDropZone, xPos, yPos, direction: string)
    {
        ...
        this.sprite.setVelocityX(Math.max(400 * rand, 95)*this.vMulti);
    ...
    collide(collidedWith : Phaser.Types.Physics.Arcade.GameObjectWithBody)
    {
        ...
        if(!this.wounded) {
            ...
            this.phaser.updateGameScore(100 - this.splats, false);
       }
    }
    ...
    public woundedHandler() // I just need these elves to stay dead
    {
        this.phaser.time.delayedCall(850, (() =>{
            ...
            this.sprite.setVelocityX(this.vMulti*(350 + 25*this.splats));
    ...
    static create(phaser : Phaser.Scene){
           phaser.anims.create({
                key: 'goblinright',
                frames: phaser.anims.generateFrameNumbers('goblin', { start: 0, end: 3 }),
                frameRate: 6,
                repeat: -1
            });
            phaser.anims.create({
                key: 'goblinleft',
                frames: phaser.anims.generateFrameNumbers('goblin', { start: 4, end: 7 }),
                frameRate: 6,
                repeat: -1
            });
            phaser.anims.create({
                key: 'goblinhurt',
                frames: phaser.anims.generateFrameNumbers('goblin', { start: 8, end: 9 }),
                frameRate: 3,
                repeat: 0
            });
    }
    ...
    static preloadAssets(phaser : Phaser.Scene){
        phaser.load.spritesheet('goblin', 
        'assets/goblin.png',
